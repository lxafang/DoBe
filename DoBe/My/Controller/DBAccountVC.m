    ...
    if (indexPath.section == 0) {
        switch (row) {
            case 0:{
                ...
                UIImage *image = [UIImage imageNamed:[DBUser sharedInstance].headImage];
            ...
            case 1:{
                ...
                [self createCellTextDetailLabel:[DBUser sharedInstance].name withSuperView:cell.contentView];
            }
                break;
            case 2:{
                [cell.textLabel setText:@"签名"];
                [self createCellTextDetailLabel:[DBUser sharedInstance].signature withSuperView:cell.contentView];
            }
                break;
        ...
        switch (row) {
            case 0:{
                ...
                [self creatCellTextField:[DBUser sharedInstance].cellphone withSuperView:cell.contentView tag:row];
            }
                break;
            case 1:{
                [cell.textLabel setText:@"邮箱"];
                [self creatCellTextField:[DBUser sharedInstance].email withSuperView:cell.contentView tag:row];
            }
                break;
            case 2:{
                [cell.textLabel setText:@"城市"];
                [self creatCellTextField:[DBUser sharedInstance].city withSuperView:cell.contentView tag:row];

            }
                break;
            case 3:{
                [cell.textLabel setText:@"地址"];
                [self creatCellTextField:[DBUser sharedInstance].address withSuperView:cell.contentView tag:row];
            }
                break;
            case 4:{
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    switch (textField.tag) {
        case 1000:{
            //电话
            [DBUser sharedInstance].cellphone = text;
        }
            break;
        case 1001:{
            [DBUser sharedInstance].email = text;
        }
            break;
        case 1002:{
            [DBUser sharedInstance].city = text;
        }
            break;
        case 1003:{
            [DBUser sharedInstance].address = text;
        }
            break;
        default:
            break;
